...
var request = http.request(options_hudmsg, function(res) {
  ...
  res.on('data', function (chunk) {
    ...
    var time= ' ';
    var histr = ' ';
    var regex_k = new RegExp(N_conf+"\(.*\) сбил");
    var regex_d = new RegExp("\(.*\) сбил .* "+N_conf);
    var regex_s = new RegExp(N_conf+"\(.*\) разбился");
    var last_reg_k;
    var last_reg_d;
    var last_reg_s;
    ...
    for (let i = 0; i <= len; i++){
      ...
      if (pivo_v1 != undefined){
        ...
        if (pivo_v1.includes(N_conf) == true && pivo_v1.includes("уничтожил") || pivo_v1.includes("сбил") == true || pivo_v1.includes("разбился") == true){var msg = pivo.msg;histr=msg;
...
console.log('Замена конфигурационных данных производится по пути: INFO/\USER/(NICK|POLK)\nНик конфигурации: '+ N_conf)
